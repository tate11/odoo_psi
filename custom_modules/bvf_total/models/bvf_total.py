...
class BVF_total(models.Model):
    ...
    definitive = fields.Boolean(string=u'DÃ©finitive')
    avoir = fields.Boolean(string='Avoir')
    
    fournisseur = fields.Many2one('fournisseur.total', string='Fournisseur', required=True)
    
    numero_facture = fields.Many2one('facture.total', size=20 , string=u'NumÃ©ro Facture', required=True, domain=[('state','=','verified')])    
    ...
    date_reception = fields.Date(string=u'Date de rÃ©ception')
    
    # calcul de l'echeance
    #date_reception_facture = fields.Date(related='date_reception', string='Date de rÃ©ception')
    date_reception_facture = fields.Char(compute='_compute_date_reception', string=u'Date de rÃ©ception de la facture', readonly=True)
    ...
    echeance = fields.Date(compute='_compute_echeance', string=u"Date d'Ã©chÃ©ance", store=True)
    ...
    date_bon_a_payer = fields.Date(string=u'Date bon Ã  payer')
    date_reglement = fields.Date(string=u'Date de rÃ¨glement')
    ...
    numero_cheque = fields.Char(string=u'NÂ° ChÃ¨que')
    date_virement = fields.Date(string='Virement du')
    numero_piece_reglement = fields.Char(string=u'NÂ° piÃ¨ce de rÃ¨glement')  
    space = fields.Char('\n', readonly=True)
    space_reglement = fields.Char('REGLEMENT', readonly=True)
        
    state = fields.Selection([
        ('recu_verifiee', u'ReÃ§ue et vÃ©rifiÃ©e'),
        ('irreguliere', u'IrrÃ©guliÃ¨res'),
        ('envoi_presc', 'Envoi aux prescripteurs'),
        ('ok_paimt_normal', 'OK pour paiement normal'),
        ('ok_paimt_immediat', u'OK pour paiement immÃ©diat'),
        ('retour_bvf_compta', 'Retour modification BVF Compta'),
        ('retour_bvf_treso', u'Retour modification BVF TrÃ©so'),
        ('retour_prescr_treso', u'Retour au prescripteur TrÃ©so'),
        ('bon_a_payer', u'Bon Ã  payer'),
        ('refacturer', 'Refacturer'),
        ('compta_gen_nd', 'Compta Gen pour ND'),
        ('recu_treso', u'ReÃ§ue TrÃ©so'),
        ('payee', u'PayÃ©e'),
    ...
    def _validate_nature(self):      
        if (not self.acompte and not self.definitive and not self.avoir):
            raise exceptions.ValidationError(u"La nature du BVF doit Ãªtre renseignÃ©e!")
    ...
    def create(self, vals):
        ...
        if  vals['montant_ttc'] == 0.0:
            raise exceptions.UserError(u'Le montant de la facture doit Ãªtre supÃ©rieur Ã  zÃ©ro.') 
        else:
    ...
    def action_retour_frnsr(self):
        ...
        return {
            'type': 'ir.actions.act_window',
                ...
                'res_model': 'change.status',
                'target': 'new',
                'context':{'state': 'irreguliere', 'default_bfv_total_id':self.id, 'default_status':u'IrrÃ©guliÃ¨re'}
                }
    ...
    def action_compta_gen_pour_nd(self):
        ...
        return {
            'type': 'ir.actions.act_window',
                ...
                'res_model': 'change.status',
                'target': 'new',
                'context':{'state': 'compta_gen_nd', 'default_bfv_total_id':self.id, 'default_status':u'Compta GÃ©n pour ND'}
                }
    ...
    def action_ok_paimt_immediat(self):
        ...
        return {
            'type': 'ir.actions.act_window',
                ...
                'res_model': 'change.status',
                'target': 'new',
                'context':{'state': 'ok_paimt_immediat', 'default_bfv_total_id':self.id, 'default_status':u'OK pour paiement immÃ©diat'}
                }
        
    def action_bon_a_payer(self):
        #self.write({'state': 'bon_a_payer'})
        if not self.numero_ef:
            raise exceptions.UserError("Veuillez renseigner le NÂ° EF")
        else:
            return {
                'type': 'ir.actions.act_window',
                'name': 'Confirmation de changement de statut',
                ...
                'res_model': 'change.status',
                'target': 'new',
                'context':{'state': 'bon_a_payer', 'default_bfv_total_id':self.id, 'default_status':u'Bon Ã  Payer'}
                }
    ...
    def action_retour_compta(self):
        ...
        return {
            'type': 'ir.actions.act_window',
                ...
                'res_model': 'change.status',
                'target': 'new',
                'context':{'state': 'ok_paimt_normal', 'default_bfv_total_id':self.id, 'default_status':u'Retour Ã  la compta'}
                }
    ...
    def action_recu_treso(self):
        ...
        return {
            'type': 'ir.actions.act_window',
                ...
                'res_model': 'change.status',
                'target': 'new',
                'context':{'state': 'recu_treso', 'default_bfv_total_id':self.id, 'default_status':u'ReÃ§ue TrÃ©so'}
                }
        
    def action_payee(self):
        #self.write({'state': 'payee'})
        fields = {u'Date de rÃ¨glement': self.date_reglement,
                  u'Date bon Ã  payer': self.date_bon_a_payer, 
                  u'NÂ° piÃ¨ce de rÃ¨glement': self.numero_piece_reglement}
        ...
        if null_fields:
            ...
            return {
                'type': 'ir.actions.act_window',
                    ...
                    'res_model': 'change.status',
                    'target': 'new',
                    'context':{'state': 'payee', 'default_bfv_total_id':self.id, 'default_status':u'PayÃ©e'}
                    }
